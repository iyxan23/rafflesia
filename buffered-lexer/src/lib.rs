    ...
    <<T as Logos<'source>>::Source as Source>::Slice: AsRef<str>,
{
    ...
    pub fn next(&mut self) -> Result<&SpannedToken<T>, error::ParseError<T, SpannedTokenOwned<T>>> {
        ...
        if self.index >= self.inner_index {
            ...
            let next_token = self.inner.next().ok_or_else(|| {
                trace!(
                    "{} * encountered eof (now {}, inner {})",
                    "  ".repeat(self.save_points.len()),
                    self.index,
                    self.inner_index
                );
            ...
            let ret = self
                .cached_tokens
                .get(self.index - self.cache_start_point)
                .ok_or_else(|| {
                    trace!(
                        "{} * encountered eof (now {}, inner {})",
                        "  ".repeat(self.save_points.len()),
    ...
    pub fn expect(
        ...
        let next = loop {
            let next: SpannedTokenOwned<T> = self
                .next()
                .map_err(|err| err.map_eof_expected(|| vec![tok.clone()]))?
                .clone()
                .into();

            if !self.blacklist.contains(&next.token) {
                break next;
            }
        };
    ...
    pub fn expect_failsafe_wo_eof(
        &mut self,
        tok: T,
    ) -> Result<Option<SpannedTokenOwned<T>>, error::ParseError<T, SpannedTokenOwned<T>>> {
        trace!(
            "{} - expecting [failsafe w/o eof] {:?}",
            "  ".repeat(self.save_points.len()),
            tok
        );
    ...
    pub fn expect_multiple_choices(
        &mut self,
        tokens: &[T],
    ) -> Result<SpannedTokenOwned<T>, error::ParseError<T, SpannedTokenOwned<T>>> {
        self.current_save_point()
    ...
    pub fn expect_peek_multiple_choices(
        &mut self,
        tokens: &[T],
    ) -> Result<SpannedTokenOwned<T>, error::ParseError<T, SpannedTokenOwned<T>>> {
        trace!(
            "{} - peeking and expecting multiple choices",
            "  ".repeat(self.save_points.len())
        );
    ...
    pub fn remove_blacklist(&mut self, tok: T) -> Result<(), ()> {
        self.blacklist
            .remove(self.blacklist.iter().position(|x| *x == tok).ok_or(())?);
    ...
    pub fn restore(&mut self) {
        let state_start_point = self.save_points.pop().expect(
    ...
    pub fn success(&mut self) {
        ...
        if self.index != self.cache_start_point {
            self.cached_tokens = self
                .cached_tokens
                .split_off(self.index - self.cache_start_point - 1);
        }
                    ...
                    if let Some(expected) = expected {
                        if expected.len() == 1 {
                            ...
                            write!(
                                ...
                                expected.iter().fold(String::new(), |acc, tok| {
                                    format!("{:?} or {:?}", acc, tok)
                                })[..3]
                                    .to_string(), // removes the trailing ` or`
                            )
                        }
                    } else {
