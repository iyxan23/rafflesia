...
fn parser<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    empty().map(|_| Definitions::default()).foldl(
        choice((
            binding()
            ...
            match result {
                (Some((typ, func_decl, func_body)), None) => {
                    if let Some(typ) = typ {
                        ...
                        acc.global_functions.insert(func_decl, func_body);
                    }
                }
                (None, Some((bind_dec, bind_body))) => {
                    acc.bindings.insert(bind_dec, bind_body);
                }
                _ => unreachable!(),
            }
...
fn binding_body<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    let function_call = select! { Token::Identifier(ident) => ident }
        .then(
            arguments
                .clone()
    ...
    let method_call_argumentless = expr
        .clone()
    ...
    choice((
        ...
        method_call_argumentless,
        method_call_with_arguments,
    ))
    .boxed()
}
...
fn binding_declaration<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    typ()
        ...
        .then(args.or_not())
        .then(ret_type)
        .map(|(((typ, ident), args), ret_type)| BindingDeclaration {
            this: typ,
            parameters: args,
            name: ident.to_string(),
...
{
    f_declaration().then(f_body().delimited_by(just(Token::LBrace), just(Token::RBrace)))
}
...
fn f_declaration<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    ident
        .then(args)
        .then(ret_type)
        .map(|((ident, args), ret_type)| FunctionDeclaration {
            this: None,
            parameters: args,
            name: ident.to_string(),
            return_type: ret_type.map(|a| a.1),
...
fn statement<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    let function_call = expr
        ...
        .then(arguments.delimited_by(just(Token::LParen), just(Token::RParen)))
        .map(|((this, ident), args)| {
...
fn expr<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    recursive(|expr| {
        ...
        let expr = choice((
            block,
            function_call.clone(),
            static_variable().map(Expression::StaticVariable),
        ))
        .boxed();

        expr.then(just(Token::Dot).ignore_then(function_call).or_not())
...
fn static_variable<'src, I: ValueInput<'src, Token = Token<'src>, Span = SimpleSpan>>(
    ...
    let literal = choice((
        ...
        select! { Token::LiteralNumber(num) => num }.map(Literal::Number),
        select! { Token::LiteralString(str) => str }.map(|str| Literal::String(str.to_string())),
    ))
